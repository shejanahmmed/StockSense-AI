    ...
    if cursor.fetchone()[0] == 0:
        sections_seeds = [
            ...
            ("product_demo", "Enterprise Dashboard Showcase",
             "Experience a fully integrated supply chain cockpit. (1) Live visual KPIs showing real-time Stock Health, Active Stockouts, Dead Capital, and Reorder Urgency. (2) B2B Cash Flow & Capital Hub showing wholesale tied-up capital and sales-at-risk. (3) Interactive Power-Grid with fuzzy search highlights and telemetry slide drawers. (4) Consolidated Multi-Item PO Planner with email generators. (5) CSV Staging & Validation Console.",
             None, 1, 40, "pitch"),
            ...
            ("tech_feature_matrix", "Application Feature Matrix",
             "Our feature map separates core edge-sovereign features from smart AI-augmented modules. Live features: CSV Staging & Validation, Facebook Prophet ML forecasting, SHAP demand driver plots, interactive Power-Grid with fuzzy search and telemetry drawers, Consolidated Multi-Item PO Planner, B2B Cash Flow & Capital Hub, and automated PDF report generation.",
             None, 1, 20, "tech"),
            ("tech_architecture", "Detailed Architecture Topology",
             "The system operates in three layers: (1) Frontend: Modern SPA built on HTML5, Vanilla CSS3, and Chart.js featuring interactive grids, CSV mapper screens, and telemetry side drawer panels. (2) Local API Core: Python FastAPI ASGI running local Facebook Prophet sub-processes, managing SQLite databases, and calculating cash flow matrices. (3) Security Boundary: Local SHA-256 hashing.",
             None, 1, 30, "tech"),
            ...
            ("tech_api", "API Routing & Endpoint Documentation",
             "StockSense AI exposes standard REST routers: (1) `auth.py`: `/api/user/login`, `/api/user/signup`. (2) `inventory.py`: `/api/inventory` GET/POST/PUT/DELETE. (3) `po.py`: `/api/purchase_orders` GET/POST. (4) `financials.py`: `/api/financials/summary` GET. (5) `chat.py`: `/api/chat`. (6) `analytics.py`: `/api/predict`, `/api/forecast/{sku}`, `/api/report` (PDF export).",
             None, 1, 60, "tech"),
            ...
            ("tech_changelog", "Application Version Changelog",
             "v1.0.0: Initial launch with local SQLite. v1.1.0: Refined mobile layouts. v1.2.0: Live `/docs` module with YC Pitch Deck, Technical Documentation, and exports. v1.3.0: Added CSV Import Staging Console (validation & inline editing), Consolidated PO Planner (supplier emails & max lead-time math), visual stock health KPIs (Stockout Count, Dead Capital), Power-Grid search highlights/telemetry drawers, and B2B Cash Flow Hub.",
             None, 1, 140, "tech")
        ]
        cursor.executemany("""
